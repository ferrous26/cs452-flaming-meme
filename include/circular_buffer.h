...
#define CB_PRODUCE					\
    size ptr = cb->cb.head;				\
    cb->buffer[ptr] = value;				\
    cb->cb.count = mod2(ptr + 1, cb->cb.length);	\
...
static inline char cbuf_count(char_buffer* const cb)                     { CB_COUNT; }
...
static inline uint32 ibuf_count(uint_buffer* const cb)                     { CB_COUNT; }
