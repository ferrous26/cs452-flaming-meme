...
\ttt{/u/cs452/tftp/ARM/marada/m3.elf}, which can be loaded with
...
\begin{center}
  \ttt{cd /u3/marada/project \&\& /u/cs444/bin/rake local}
\end{center}
...
\ttt{Master}, the transport level of the hierarchy, was now
...
The \ttt{Path Finding Administrator} would be contacted when a new
route was required. The administrator would allocate a \ttt{Worker}
task to perform the actual work and then the \ttt{Worker} would
contact the \ttt{Master} once the route had been computed.

\ttt{Master} tasks would hold onto \ttt{Worker} tasks for the duration
of a path finding operation, and only release (\ttt{Reply} to) the
\ttt{Worker} when a new route was requested. This was done to share
memory from the \ttt{Worker} to \ttt{Master} in order to avoid
...
\ttt{ch <chaser> <target>} command with two train numbers. For
example, the command \ttt{ch 45 56} will cause train 45 to chase train
...
\ttt{tr <train> <speed>} and \ttt{sw <switch> <state>} commands from
...
\ttt{Master} tasks to communicate with each other. The courier is able
to ask a \ttt{Master} task its position on the track and report the
location.
...
\ttt{Path Administrator} when finding a route to Max. Path finding
...
command \ttt{hr 58} in order to toggle the horn from train 58.
